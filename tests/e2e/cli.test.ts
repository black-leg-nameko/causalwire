...
describe('CLI states',()=>{
  it('runs the success demo and writes real artifacts',()=>{const dir=mkdtempSync(join(tmpdir(),'cw-e2e-'));dirs.push(dir);const result=spawnSync(process.execPath,[tsx,'src/cli.ts','demo','--out-dir',dir],{encoding:'utf8'});expect(result.status).toBe(0);expect(result.stdout).toContain('FIRST BREAK D004');expect(readFileSync(join(dir,'stuck-tool.html'),'utf8')).toContain('Causal sequence');expect(readFileSync(join(dir,'stuck-tool.svg'),'utf8')).toContain('FIRST BREAK');});
