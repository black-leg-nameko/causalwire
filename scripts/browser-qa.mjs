...
for(const item of cases){
  ...
  const metrics=await page.evaluate(()=>{const text=document.body?.textContent??document.documentElement.textContent??'';return {title:document.title,scrollWidth:document.documentElement.scrollWidth,innerWidth:window.innerWidth,scrollX:window.scrollX,scrollY:window.scrollY,bodyText:text.slice(0,300),firstBreak:text.includes('FIRST BREAK'),empty:text.includes('No JSON-RPC exchanges found')};});
  results.push({...item,...metrics,consoleErrors,failedRequests,pass:metrics.scrollWidth<=metrics.innerWidth&&consoleErrors.length===0&&failedRequests.length===0});await page.close();
}
